<!DOCTYPE html>
<html lang="en" class="<?= (isset($theme) && $theme == 'dark') ? 'dark' : '' ?>">
<head>
    <meta charset="utf-8"/>
    <meta content="width=device-width, initial-scale=1.0" name="viewport"/>
    <title><?= $title ?> - Distrik Navigasi Tanjungpinang</title>
    <script src="https://cdn.tailwindcss.com?plugins=forms,container-queries"></script>
    <link href="https://fonts.googleapis.com/css2?family=Public+Sans:wght@300;400;500;600;700;800&display=swap" rel="stylesheet"/>
    <link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:wght,FILL@100..700,0..1&display=swap" rel="stylesheet"/>
    <script>
        tailwind.config = { darkMode: "class", theme: { extend: { colors: { primary: "#0e4c92", secondary: "#fbb03b", "background-light": "#f8fafc", "background-dark": "#0f172a", "surface-light": "#ffffff", "surface-dark": "#1e293b", "sidebar-bg": "#ffffff", "sidebar-active": "#eff6ff", }, fontFamily: { display: ["Public Sans", "sans-serif"], }, boxShadow: { 'soft': '0 4px 6px -1px rgba(0, 0, 0, 0.02), 0 2px 4px -1px rgba(0, 0, 0, 0.02)', } }, }, }
    </script>
    <style>
...
<header class="h-16 bg-white dark:bg-surface-dark border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 z-20 shadow-sm flex-shrink-0">
    <div class="flex items-center gap-4">
        <div class="flex items-center gap-3">
            <div class="size-10 rounded-lg bg-primary flex items-center justify-center text-white shrink-0 shadow-lg shadow-blue-900/20"><span class="material-symbols-outlined text-2xl">anchor</span></div>
...
<a class="flex items-center gap-3 px-3 py-2 rounded-lg bg-primary text-white font-medium shadow-md shadow-primary/20 transition-all group" href="<?= base_url('admin/settings') ?>">
                <span class="material-symbols-outlined filled">settings_applications</span><span class="text-sm">System Settings</span>
            </a>
</nav>
            ...
            <form action="<?= base_url('admin/settings/save') ?>" method="POST" class="max-w-5xl mx-auto space-y-8">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 pb-4 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <div class="flex gap-3">
                        <a href="<?= base_url('admin/settings/logs') ?>" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-surface-dark border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 shadow-sm transition-all">
                            <span class="material-symbols-outlined text-lg">history</span> View Logs
                        </a>
                ...
                <?php if(session()->getFlashdata('success')): ?>
                    <div class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-lg flex items-center gap-2 font-bold shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-white dark:bg-surface-dark rounded-xl p-5 shadow-soft border border-slate-200 dark:border-slate-700 flex flex-col">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="p-2 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-lg"><span class="material-symbols-outlined">dns</span></div>
                            <h3 class="font-semibold text-slate-800 dark:text-white">Database Status</h3>
                        </div>
                        <div class="flex items-end justify-between mt-auto">
                            <div><div class="text-2xl font-bold text-slate-900 dark:text-white">Healthy</div><div class="text-xs text-slate-500">Last check: Just now</div></div>
                            <div class="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
                        </div>
                    </div>
                    <div class="bg-white dark:bg-surface-dark rounded-xl p-5 shadow-soft border border-slate-200 dark:border-slate-700 flex flex-col">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="p-2 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg"><span class="material-symbols-outlined">hard_drive</span></div>
                            <h3 class="font-semibold text-slate-800 dark:text-white">Storage Usage</h3>
                        </div>
                        <div class="mt-auto">
                            <div class="flex justify-between text-xs font-medium text-slate-600 dark:text-slate-300 mb-1"><span>245 GB Used</span><span>1 TB Total</span></div>
                            <div class="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2"><div class="bg-blue-600 h-2 rounded-full" style="width: 24%"></div></div>
                        </div>
                    </div>
                    <div class="bg-white dark:bg-surface-dark rounded-xl p-5 shadow-soft border border-slate-200 dark:border-slate-700 flex flex-col">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="p-2 bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-lg"><span class="material-symbols-outlined">memory</span></div>
                            <h3 class="font-semibold text-slate-800 dark:text-white">Server Load</h3>
                        </div>
                        <div class="mt-auto">
                            <div class="text-2xl font-bold text-slate-900 dark:text-white">12%</div><div class="text-xs text-slate-500">Optimal performance</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="bg-white dark:bg-surface-dark rounded-xl shadow-soft border border-slate-200 dark:border-slate-700">
                            <div class="p-6 border-b border-slate-100 dark:border-slate-800">
                                <h3 class="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><span class="material-symbols-outlined text-primary">badge</span> Office Identity</h3>
                        ...
                        <div class="bg-white dark:bg-surface-dark rounded-xl shadow-soft border border-slate-200 dark:border-slate-700">
                            <div class="p-6 border-b border-slate-100 dark:border-slate-800">
                                <h3 class="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><span class="material-symbols-outlined text-primary">notifications_active</span> Notification Preferences</h3>
                            ...
                            <div class="p-6 space-y-4">
                                <div class="flex items-center justify-between py-2">
                                    ...
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input name="notif_email" type="checkbox" value="1" <?= $notif_email == '1' ? 'checked' : '' ?> class="sr-only peer"/>
                                ...
                                <div class="flex items-center justify-between py-2 border-t border-slate-100 dark:border-slate-700/50">
                                    ...
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input name="notif_alerts" type="checkbox" value="1" <?= $notif_alerts == '1' ? 'checked' : '' ?> class="sr-only peer"/>
                                        <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                                    </label>
                                </div>
                                <div class="flex items-center justify-between py-2 border-t border-slate-100 dark:border-slate-700/50">
                                    ...
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input name="notif_expiring" type="checkbox" value="1" <?= $notif_expiring == '1' ? 'checked' : '' ?> class="sr-only peer"/>
                                        <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                                    </label>
                                </div>
                            </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white dark:bg-surface-dark rounded-xl shadow-soft border border-slate-200 dark:border-slate-700">
                            <div class="p-6 border-b border-slate-100 dark:border-slate-800">
                                <h3 class="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><span class="material-symbols-outlined text-primary">palette</span> Appearance</h3>
                            </div>
                            <div class="p-6">
                                <div class="grid grid-cols-2 gap-4">
                                    <label class="cursor-pointer group">
                                        <input class="peer sr-only" name="theme" type="radio" value="light" <?= $theme == 'light' ? 'checked' : '' ?>/>
                                        <div class="relative overflow-hidden rounded-lg border-2 border-slate-200 peer-checked:border-primary peer-checked:ring-2 peer-checked:ring-primary/20 dark:border-slate-700 bg-slate-50 p-2 group-hover:bg-slate-100 transition-all">
                                            <div class="space-y-2">
                                                <div class="h-2 w-full bg-white rounded"></div><div class="h-2 w-3/4 bg-white rounded"></div>
                                                <div class="flex gap-2"><div class="h-4 w-4 bg-primary rounded-full"></div><div class="h-4 w-full bg-white rounded"></div></div>
                                            </div>
                                            <div class="mt-3 text-center text-sm font-medium text-slate-700">Light</div>
                                            <div class="absolute right-2 top-2 hidden text-primary peer-checked:block"><span class="material-symbols-outlined text-lg">check_circle</span></div>
                                        </div>
                                    </label>
                                    <label class="cursor-pointer group">
                                        <input class="peer sr-only" name="theme" type="radio" value="dark" <?= $theme == 'dark' ? 'checked' : '' ?>/>
                                        <div class="relative overflow-hidden rounded-lg border-2 border-slate-200 peer-checked:border-primary peer-checked:ring-2 peer-checked:ring-primary/20 dark:border-slate-700 bg-slate-900 p-2 group-hover:bg-slate-800 transition-all">
                                            <div class="space-y-2">
                                                <div class="h-2 w-full bg-slate-800 rounded"></div><div class="h-2 w-3/4 bg-slate-800 rounded"></div>
                                                <div class="flex gap-2"><div class="h-4 w-4 bg-primary rounded-full"></div><div class="h-4 w-full bg-slate-800 rounded"></div></div>
                                            </div>
                        ...
                        <div class="bg-white dark:bg-surface-dark rounded-xl shadow-soft border border-slate-200 dark:border-slate-700">
                            <div class="p-6 border-b border-slate-100 dark:border-slate-800">
                                <h3 class="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><span class="material-symbols-outlined text-primary">database</span> Database Management</h3>
                            </div>
                            <div class="p-6 space-y-4">
                               <div class="p-4 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700">
    <h4 class="text-sm font-semibold text-slate-900 dark:text-white mb-1">Database Backup</h4>
    <p class="text-xs text-slate-500 mb-3">Unduh struktur SQL untuk keperluan migrasi atau keamanan.</p>
    <div class="flex gap-2">
        <a href="<?= base_url('admin/settings/backup') ?>" class="flex-1 py-2 px-3 text-center bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-xs font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors">
            Download SQL
        </a>
        <button type="button" onclick="alert('Fitur Restore dinonaktifkan di mode demo.')" class="flex-1 py-2 px-3 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-xs font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors opacity-50 cursor-not-allowed">
            Restore
        </button>
    </div>
</div>

<a href="<?= base_url('admin/settings/backup') ?>" class="w-full py-2.5 bg-primary text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors shadow-sm flex items-center justify-center gap-2 mt-4">
...
<div class="pt-4 mt-4 border-t border-slate-100 dark:border-slate-700/50">
    <a href="<?= base_url('admin/settings/reset') ?>" onclick="return confirm('BAHAYA: Apakah Anda yakin ingin menghapus seluruh data surat? (Data user & lokasi akan tetap aman)')" class="w-full py-2 text-red-600 dark:text-red-400 text-xs font-medium hover:bg-red-50 dark:hover:bg-red-900/10 rounded-lg transition-colors flex items-center justify-center gap-1">
